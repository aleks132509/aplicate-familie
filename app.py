...
def get_initial_apple_watch():
    ...
    while curr <= end_date:
        aw_rows.append({
            "Dată": curr.strftime("%d.%m.%Y"),
            "Pași": steps_base + (curr.day * 120) % 2500,
            "Calorii Active": 450 + (curr.day * 40) % 300,
            "Somn (ore)": round(7.2 + ((curr.day % 3) * 0.3), 1),
            "HRV (ms)": 48 + (curr.day * 3) % 20,
            "SpO2 (%)": 98,
            "Puls Mediu": 72 + (curr.day % 5)
        })
        curr += timedelta(days=1)
        
    df_init_aw = pd.DataFrame(aw_rows)
    df_init_aw.to_csv(APPLE_WATCH_FILE, index=False)
    return df_init_aw
    ...
    try:
        ...
        if ultima_data is None or ultima_data < azi:
            ...
            if not df_cron.empty:
                ...
                if os.path.exists(APPLE_WATCH_FILE):
                    attachments["apple_watch_data.csv"] = APPLE_WATCH_FILE
                ...
                succes, _ = trimite_email_cu_multiple_atasamente(
                    ...
                    mesaj=f"Salut!\n\nAcesta este backup-ul tău zilnic automat generat la data de {azi.strftime('%d.%m.%Y')}.\nSunt atașate fișierele CSV cu datele medicale, schema de tratament, programările și datele Apple Watch.\n\nHealthTrack Pro System",
...
# ==========================================
# ORGANIZARE TAB-URI (INCLUSIV APPLE WATCH)
# ==========================================
...
with tab_dict["⌚ Apple Watch"]:
    ...
    if not aw_df.empty:
        ...
        with sub_tab_aw_act:
            st.markdown("ℹ️ **Evoluție Pași și Calorii Active înregistrate de Apple Watch**")
            ...
            fig_aw_act.add_trace(go.Bar(
                x=x_aw_dates, y=steps_vals, name="Pași",
                marker_color="#38bdf8", opacity=0.85
            ))
            fig_aw_act.update_layout(
                template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(l=40, r=40, t=40, b=80), xaxis=dict(tickangle=-45)
            )
            st.plotly_chart(fig_aw_act, use_container_width=True)

            fig_aw_cal = go.Figure()
            fig_aw_cal.add_trace(go.Scatter(
                x=x_aw_dates, y=cal_vals, mode="lines+markers", name="Calorii Active (kcal)",
                line=dict(color="#f59e0b", width=2.5), marker=dict(size=8)
            ))
            fig_aw_cal.update_layout(
                template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(l=40, r=40, t=40, b=80), xaxis=dict(tickangle=-45)
            )
        ...
        with sub_tab_aw_sleep:
            st.markdown("ℹ️ **Evoluție Somn (ore), HRV și SpO2**")
            ...
            fig_aw_sleep.add_trace(go.Scatter(
                x=x_aw_dates, y=sleep_vals, mode="lines+markers", name="Somn (ore)",
                line=dict(color="#10b981", width=2.5), marker=dict(size=8)
            ))
            fig_aw_sleep.add_trace(go.Scatter(
                x=x_aw_dates, y=hrv_vals, mode="lines+markers", name="HRV (ms)",
                line=dict(color="#8b5cf6", width=2.5), marker=dict(size=8)
            ))
            fig_aw_sleep.update_layout(
                template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                margin=dict(l=40, r=40, t=40, b=80), xaxis=dict(tickangle=-45)
            )
        ...
        if is_admin:
            st.markdown("---")
            st.markdown("#### ➕ Adaugă / Actualizează Valori Apple Watch (Administrator)")
            with st.form("form_add_apple_watch"):
                aw_date_inp = st.date_input("Dată", value=date.today(), key="aw_date_input_admin")
                ...
                if st.form_submit_button("💾 Salvează Datele Apple Watch", type="primary"):
                    ...
                    st.session_state.apple_watch_df.to_csv(APPLE_WATCH_FILE, index=False)
                    st.success(f"Datele Apple Watch pentru {d_str_aw} au fost salvate cu succes!")
                    trigger_rerun()
    else:
        st.info("Nu există date Apple Watch înregistrate.")
                    ...
                    if not destinatar_auto:
                        ...
                        if trimis_ok > 0:
                            succes, rez = trimite_email_cu_multiple_atasamente(destinatar_auto, "🔔 Notificare Programare Medicală", mesaj_final, {
                                ...
                                "apple_watch_data.csv": APPLE_WATCH_FILE
                            })
                            if succes:
        ...
        if st.button("🔌 Forțează Trimite Backup Complet Acum (Toate Fișierele)", type="primary"):
            ...
            if not test_dest:
                ...
                success_t, msg_t = trimite_email_cu_multiple_atasamente(
                    ...
                    "Salut! Acesta este un email de test forțat cu toate fișierele atașate (date medicale, tratament, programări și Apple Watch).", 
                    attachments
                )
...
with tab_dict["📄 Raport PDF"]:
    ...
    with col_opt1:
        ...
        opt_aw_chart = st.checkbox("Include Grafice Apple Watch (Pași, Somn) ⌚", value=True)
    with col_opt2:
        opt_puls = st.checkbox("Include Grafic Puls 💓", value=True)
        opt_tabele = st.checkbox("Include Tabelul Centralizator Medical 📋", value=True)
        opt_aw_table = st.checkbox("Include Tabelul Apple Watch ⌚", value=True)
