...
# ==========================================
# CITIRE TABEL CU ANTET DUBLU (DIN IMAGINE)
# ==========================================
...
def load_and_clean_data(url):
  try:
    ...
    # Căutăm rândul exact unde apare "Dată" în prima coloană (Rândul 2 din antetul combinat)
    header_row_idx = None
...
if not df.empty:
  ...
  if date_col and date_col in df.columns:
    # Conversie strictă pentru formatul DD.MM.YYYY din imagine
...
with tab_jurnal:
  ...
  if not df.empty and date_col is not None and date_col in df.columns:
    ...
    # Căutare inteligentă a coloanelor specifice antetului tău
    col_glic = next(
        (c for c in cols if "glic" in remove_diacritics(str(c)).lower() or "inainte" in remove_diacritics(str(c)).lower()),
        None,
    )
    col_sis = next(
